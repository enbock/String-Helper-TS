...
export default class StringHelper {
    ...
    public static replaceUmlaut(input: string): string {
        let result: string = input;
        for (let i: number = 0; i < StringHelper.diacriticsMap.length; i++) {
            result = result.replace(StringHelper.diacriticsMap[i].letters, StringHelper.diacriticsMap[i].base);
        }
    ...
    private static compareByProperty(a: Object, b: Object, property: string): number {
        ...
        const upperA: string = StringHelper.replaceUmlaut(String((a as any)[property])).toUpperCase();
        const upperB: string = StringHelper.replaceUmlaut(String((b as any)[property])).toUpperCase();
        if (upperA < upperB) {
            return -1;
        }
        if (upperA > upperB) {
